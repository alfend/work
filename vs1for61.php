...
";
    ...
    $query1 = "SELECT `id`, `cad_num`,`adres`,`liter`, `new_adres`,`kod_kladr`,`new_dom`,`new_kv`,`new_kom`,`area`,`name` FROM `egrp_kladr_new` where `kod_kladr` is not null and ((`new_dom`<>'')or(`new_dom`>10000)) and `id`>'702983'";//and `id`<1200000
    $sql1 = mysql_query($query1) or die(mysql_error());
    while ($row1 = mysql_fetch_assoc($sql1)) {
        $query2 = "SELECT `id`, `adres`,`liter`, `new_adres`,`kod_kladr`,`new_dom`,`new_kv`,`new_kom`,`area`,`name` FROM `gkn_kladr_new` where `cad_num`='" . $row1['cad_num'] . "' and `kod_kladr` is not null and ((`new_dom`<>'')or(`new_dom`>10000)) limit 1";
