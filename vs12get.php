...
<div id="basis">
    ...
    if ($vfile) {
        $handle = @fopen("files/vs12get-" . $id_user . ".txt", "w");
...
";
    ...
    $query1 = "SELECT `id`, `cad_num`,`adres`,`liter`, `new_adres`,`kod_kladr`,`new_dom`,`new_kv`,`new_kom`,`area`,`name` FROM `egrp_kladr_new` where `kod_kladr` is not null and `id`>=" . $egrpbeg . " and `id`<=" . $egrpend . " ";//and `id`<1200000 limit 10000
...
';
    ...
    if ($vfile) {
        fclose($handle);
        print('<a href="files/vs12get-' . $id_user . '.txt">Файл</a>');
    } else {
        print("</table>");
    };
    gc_collect_cycles();
