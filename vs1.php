...
<div id="basis">
    ...
    if ($vfile) {
        $handle = @fopen("files/vs1-" . $id_user . ".txt", "w");
...
";
    ...
    $query1 = "SELECT `id`, `cad_num`,`adres`,`liter`, `new_adres`,`kod_kladr`,`new_dom`,`new_kv`,`new_kom`,`area`,`name`,`type` FROM `egrp_kladr_new` where `kod_kladr` is not null ";//and `id`<1200000
    $sql1 = mysql_query($query1) or die(mysql_error());
    while ($row1 = mysql_fetch_assoc($sql1)) {
        $query2 = "SELECT `id`, `adres`,`liter`, `new_adres`,`kod_kladr`,`new_dom`,`new_kv`,`new_kom`,`area`,`name`,`type` FROM `gkn_kladr_new` where `cad_num`='" . $row1['cad_num'] . "' and `kod_kladr` is not null limit 1";
        ...
        if ($row2['id'] != '') {
            ...
            if ((($row1['new_kv'] != $row2['new_kv']) and ($row2['new_kv'] == '') and ((strpos(mb_strtoupper($row2['name'],
                                "UTF-8"), 'КВАРТИРА') > -1) or (strpos(mb_strtoupper($row2['type'], "UTF-8"),
                                '002002002000') > -1))) or (($row1['new_kv'] != $row2['new_kv']) and ($row1['new_kv'] == '') and ((strpos(mb_strtoupper($row1['name'],
                                "UTF-8"), 'КВАРТИРА') > -1) or (strpos(mb_strtoupper($row1['type'], "UTF-8"),
                                'ПОМЕЩЕНИЕ') > -1)))) {
                $booladr = true;
            };
...
';
    ...
    if ($vfile) {
        fclose($handle);
        print('<a href="files/vs1-' . $id_user . '.txt">Файл</a>');
    } else {
        print("</table>");
    };
    gc_collect_cycles();
