...
<div id="basis">
    ...
    if ($vfile) {
        $handle = @fopen("files/vs1kv-" . $id_user . ".txt", "w");
...
";
    ...
    $query1 = "SELECT `id`, `cad_num`,`adres`,`liter`, `new_adres`,`kod_kladr`,`new_dom`,`new_kv`,`new_kom`,`area`,`name` FROM `egrp_kladr_new` where `kod_kladr` is not null order by `id` limit 1000";//and `id`<1200000
...
//		if((($row1['new_kv']!=$row2['new_kv'])and($row2['new_kv']=='')and(strpos(mb_strtoupper($row2['name'],"UTF-8"),'КВАРТИРА')>-1))or(($row1['new_kv']!=$row2['new_kv'])and($row1['new_kv']=='')and(strpos(mb_strtoupper($row1['name'],"UTF-8"),'КВАРТИРА')>-1))){$booladr=true;};
