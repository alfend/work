...
<div id="basis">
    ...
    if ($vfile) {
        $handle = @fopen("files/vs4-" . $id_user . ".txt", "w");
        $strfile = "id ЕГРП|КН ЕГРП|Адрес ЕГРП.
";
        fwrite($handle, $strfile);
    } else {
        print('<tr><td>id ЕГРП</td></td><td>КН ЕГРП</td><td>Адрес ЕГРП</td></tr>');
    };
    ...
    $query1 = "SELECT `id`, `cad_num`,`adres`,`liter`, `new_adres` FROM `egrp_kladr_new` ";//and `id`<1200000
    $sql1 = mysql_query($query1) or die(mysql_error());
    while ($row1 = mysql_fetch_assoc($sql1)) {//$adres=preg_replace('/[\s.,;]/',' ',);     mb_strtoupper($row1['liter'],"UTF-8")
        $query2 = "SELECT `id` FROM `gkn_kladr_new` where `cad_num`='" . $row1['cad_num'] . "'";
...
';
    ...
    if ($vfile) {
        fclose($handle);
        print('<a href="files/vs4-' . $id_user . '.txt">Файл</a>');
    } else {
        print("</table>");
    };
    gc_collect_cycles();
